...
class fcn8s(nn.Module):
    def __init__(self, vgg16_rmfc, n_class):
        ...
        self.auxpred512 = nn.Sequential(
            nn.Conv2d(in_channels=512, out_channels=n_class, kernel_size=1, stride=1, padding=0)
        )
        self.auxpred256 = nn.Sequential(
            nn.Conv2d(in_channels=256, out_channels=n_class, kernel_size=1, stride=1, padding=0)
        )
    ...
    def forward(self, x):
        ...
        # 7 * 7 * 512
        x4 = self.vgg16_rmfc.features[0:24](x)  # exclude layer 24
        # 14 * 14 * 512
        x4 = self.auxpred512(x4)
        # 14 * 14 * n_class
        x3 = self.vgg16_rmfc.features[0:17](x)  # exclude layer 17
        # 28 * 28 * 256
        x3 = self.auxpred256(x3)
        # 28 * 28 * n_class
...
if __name__ == '__main__':
    ...
    # for param in list(vgg16_rmfc.features.parameters()):
